...
- The design system is NOT a pre-existing JS repo: it lives in `design/design-system/` and was hand-built (2026-07-09) as a web translation of the launcher's Avalonia styling. The launcher stays the source of truth - `design/design-system/build.mjs` copies fonts and inlines the LED button art from `src/Ksp2Redux.Tools.Launcher/Assets/` at build time, and `src/tokens.css` mirrors `Colors.axaml`/`Radii.axaml` by hand. If the launcher palette changes, update tokens.css to match.
- Build: `npm run build` inside `design/design-system/` (esbuild bundle + tsc declarations + CSS stitch). Node 22 / npm on Windows.
- Converter invocation from repo root: `--node-modules ./design/design-system/node_modules --entry ./design/design-system/dist/index.js`.
...
- Button art and fonts are copied from the launcher's Assets at build time; if those files move or get renamed, `design/design-system/build.mjs` fails loudly.
...
- First upload was blocked on DesignSync authorization (no interactive /design-login in this environment) - if `design/design-sync.config.json` has no `projectId`, no project exists yet and the first upload still needs the full §1 target-settlement flow.
